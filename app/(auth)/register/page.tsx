        ...
        <div className="rg-right">
          <div className="rg-card">
            ...
            <div style={{ background: '#f8fafc', borderRadius: 16, padding: '20px', marginBottom: 24 }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', marginBottom: 14, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Langkah selanjutnya</p>
              {[
                ...
                <div key={s.n} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                  <div style={{ width: 26, height: 26, borderRadius: '50%', flexShrink: 0, background: '#0d3b2e', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800 }}>{s.n}</div>
            ...
            <Link href="/login" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '14px', borderRadius: 14, background: '#0d3b2e', color: 'white', fontSize: 15, fontWeight: 700, textDecoration: 'none', boxShadow: '0 4px 20px rgba(13,59,46,0.25)' }}>
              Ke Halaman Login <ArrowRight size={16} />
            </Link>

            <p style={{ fontSize: 13, color: '#94a3b8', textAlign: 'center', marginTop: 16 }}>
              Email salah?{' '}
              <button onClick={() => setStep('form')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#0d3b2e', fontWeight: 700, fontSize: 13, padding: 0, fontFamily: 'inherit' }}>
                Daftar ulang
              </button>
            </p>
          </div>
        ...
        <div className="rg-right">
          <div className="rg-card">
            ...
            <div className="rg-mobile-logo">
              <Link href="/">
                <img src="/logo_findor.jpg" alt="Findor" style={{ height: 38, width: 'auto', objectFit: 'contain', borderRadius: 8 }} />
              </Link>
            </div>

            <div style={{ marginBottom: 24 }}>
              <h1 className="rg-heading">Buat Akun Gratis</h1>
              <p style={{ fontSize: 14, color: '#94a3b8', textAlign: 'center', marginTop: 4 }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {form.password && (
                  <div style={{ marginTop: 8 }}>
                    <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                      {[1,2,3].map(i => (
                        <div key={i} style={{ flex: 1, height: 3, borderRadius: 99, background: passStrength >= i ? passColors[passStrength] : '#e5e7eb', transition: 'background 0.3s' }} />
                      ))}
                    </div>
                    <p style={{ fontSize: 11, color: passColors[passStrength], fontWeight: 600 }}>{passLabels[passStrength]}</p>
                  </div>
                )}
              </Field>
              ...
              >
                {loading ? <><Spinner /> Membuat akun...</> : <>Buat Akun Gratis <ArrowRight size={16} /></>}
              </button>
            </form>
...
function LeftPanel() {
  return (
    <div className="rg-left">
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'url(https://images.unsplash.com/photo-1519167758481-83f550bb49b3?w=900&q=80)', backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.12 }} />
      ...
      <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '48px 40px' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: '16px 20px', backdropFilter: 'blur(10px)' }}>
          ...
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', lineHeight: 1.6, margin: 0 }}>
            &ldquo;Findor sangat membantu! Saya bisa temukan vendor dekorasi impian dalam 10 menit.&rdquo;
          </p>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label style={{ display: 'block', fontSize: 13, fontWeight: 700, color: '#374151', marginBottom: 7, letterSpacing: '0.01em' }}>{label}</label>
      {children}
    </div>
  );
}
  ...
  return (
    <style>{`
      ...
      @media (max-width: 680px) {
        ...
        .rg-mobile-logo { display: flex; }
        .rg-heading      { font-size: 22px; }
      }

      @media (max-width: 400px) {
        .rg-right { padding: 20px 12px 36px; }
        .rg-card  { padding: 24px 16px 32px; }
