...
function MobileStickyBar({ onWhatsApp, onBook }: { onWhatsApp: () => void; onBook: () => void }) {
  return (
    <div className="mobile-sticky-bar">
      <button onClick={onWhatsApp} className="mobile-sticky-wa">
        <MessageCircle size={17} /> WhatsApp
      </button>
      <button onClick={onBook} className="mobile-sticky-book">
        <Calendar size={17} /> Booking
      </button>
    </div>
  );
}
...
export default function VendorDetailPage() {
  ...
  const handleBookNow = (service?: Service) => {
    if (!vendor) return;
    const svc = service ?? (vendor.services.length > 0 ? vendor.services[0] : null);
    if (svc) { setSelectedService(svc); setShowBookingModal(true); }
    else setActiveTab('layanan');
  };
  ...
  if (loading) {
    return (
      <main style={{ minHeight: '100vh', background: '#f7f7f5' }}>
        <Navbar />
        <div className="vendor-layout" style={{ maxWidth: 1200, margin: '0 auto', padding: '100px 24px 60px' }}>
          {[...Array(4)].map((_, i) => (
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f7f7f5' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '90px 24px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#94a3b8', marginBottom: 24, flexWrap: 'wrap' }}>
      ...
      <div className="vendor-layout">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ background: 'white', borderRadius: 20, padding: '28px 24px 24px', border: '1px solid #f1f5f9' }}>
            <div className="vendor-header-row">
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                  {vendor.is_verified && (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#dcfce7', color: '#15803d', fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 999 }}>
                ...
                <h1 className="vendor-title">{vendor.store_name}</h1>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: '#64748b' }}>
                    ...
                    {vendor.address && <span className="vendor-address-desktop"> · {vendor.address}</span>}
                  </div>
                </div>
              </div>
            ...
            {vendor.description && (
              <p style={{ fontSize: 14.5, color: '#475569', lineHeight: 1.75, borderTop: '1px solid #f1f5f9', paddingTop: 16, marginTop: 16 }}>
                {vendor.description}
              </p>
            )}
            ...
            <div className="vendor-badges">
              {[
                ...
                <div key={label} className="vendor-badge-item">
            ...
            <div className="mobile-cta-inline">
              <button onClick={handleWhatsApp} className="mobile-inline-wa">
                <MessageCircle size={16} /> Hubungi via WhatsApp
              </button>
              <button onClick={() => handleBookNow()} className="mobile-inline-book">
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid #f1f5f9', overflowX: 'auto' }}>
              {(['layanan', 'ulasan', 'info'] as const).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  style={{ flex: 1, padding: '16px 12px', fontSize: 14, fontWeight: activeTab === tab ? 700 : 500, color: activeTab === tab ? '#0d3b2e' : '#9ca3af', background: 'none', border: 'none', borderBottom: activeTab === tab ? '2.5px solid #0d3b2e' : '2.5px solid transparent', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', transition: 'all 0.18s' }}>
                  ...
                  {vendor.services.length === 0 ? (
                    ...
                    return (
                      <div key={svc.id} style={{ borderRadius: 14, borderWidth: '1.5px', borderStyle: 'solid', borderColor: isExpanded ? '#0d3b2e' : '#e5e7eb', overflow: 'hidden', transition: 'border-color 0.18s' }}>
                        ...
                        {isExpanded && (
                          <div style={{ padding: '14px 18px 18px', borderTop: '1px solid #e5e7eb' }}>
                            ...
                            <button onClick={() => handleBookNow(svc)}
                              style={{ width: '100%', padding: '12px 0', borderRadius: 10, background: '#0d3b2e', color: 'white', fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'inherit' }}>
              ...
              {activeTab === 'info' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[
                    ...
                    <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #f1f5f9', gap: 12 }}>
                      <span style={{ fontSize: 13.5, color: '#94a3b8', fontWeight: 500, flexShrink: 0 }}>{label}</span>
                      <span style={{ fontSize: 13.5, color: '#0f172a', fontWeight: 600, textAlign: 'right' }}>{value}</span>
                    </div>
                  ))}
        ...
        <div className="vendor-sidebar">
          <div style={{ background: 'white', borderRadius: 20, padding: '24px 22px', border: '1px solid #f1f5f9', boxShadow: '0 4px 24px rgba(0,0,0,0.07)' }}>
            ...
            <button onClick={handleWhatsApp}
              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '14px 0', borderRadius: 12, background: '#16a34a', color: 'white', fontSize: 15, fontWeight: 700, border: 'none', cursor: 'pointer', marginBottom: 10, fontFamily: 'inherit' }}>
              <MessageCircle size={18} /> Hubungi via WhatsApp
            </button>

            <button onClick={() => handleBookNow()}
              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '13px 0', borderRadius: 12, background: '#0d3b2e', color: 'white', fontSize: 15, fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
            ...
            {bookingSuccess && (
              <div style={{ marginTop: 12, padding: '10px 14px', background: '#f0fdf4', borderRadius: 10, border: '1px solid #bbf7d0', fontSize: 13, color: '#15803d', display: 'flex', gap: 8, alignItems: 'center' }}>
                <CheckCircle size={15} /> Booking berhasil! Cek <Link href="/bookings" style={{ color: '#0d3b2e', fontWeight: 700 }}>Bookings</Link>.
              </div>
            )}
      ...
      <MobileStickyBar onWhatsApp={handleWhatsApp} onBook={() => handleBookNow()} />

      {showBookingModal && selectedService && (
        <BookingModal vendor={vendor} service={selectedService} onClose={() => setShowBookingModal(false)} onSuccess={() => setBookingSuccess(true)} />
      )}
      ...
      <style>{`
        ...
        .mobile-cta-inline  { display: none; }
        .mobile-calendar    { display: none; }
        .vendor-address-desktop { display: inline; }
        ...
        @media (max-width: 1024px) {
          ...
          .mobile-sticky-wa {
            ...
            padding: 13px 0; border-radius: 12px; background: #16a34a;
            color: white; font-size: 14px; font-weight: 700;
            border: none; cursor: pointer; font-family: inherit;
          }
          .mobile-sticky-book {
            flex: 1; display: flex; align-items: center; justify-content: center; gap: 8px;
            padding: 13px 0; border-radius: 12px; background: #0d3b2e;
            color: white; font-size: 14px; font-weight: 700;
            border: none; cursor: pointer; font-family: inherit;
          }
        ...
        @media (max-width: 768px) {
          .vendor-layout { padding: 0 12px 120px; gap: 16px; }
          .vendor-title { font-size: 22px; }
          .vendor-badges { gap: 8px; }
          .vendor-badge-item { flex: 1 1 140px; padding: 9px 12px; }
          .tab-content { padding: 18px 16px; }
          .rating-summary { grid-template-columns: 1fr; gap: 16px; padding: 16px; }
          .vendor-address-desktop { display: none; }
        }

        @media (max-width: 640px) {
          .vendor-layout { padding: 0 12px 110px; }
          .vendor-title { font-size: 20px; }
          .vendor-header-row { gap: 10px; }
          .tab-content { padding: 16px 14px; }
          .vendor-badges { flex-direction: column; gap: 8px; }
          .vendor-badge-item { flex: unset; }
          .mobile-sticky-bar { padding: 10px 12px; gap: 8px; }
          .mobile-sticky-wa, .mobile-sticky-book { font-size: 13px; padding: 12px 0; }
        }
