...
interface VendorProfile { id: string; store_name: string; is_verified: boolean; }
...
function Toast({ msg, type, onClose }: { msg: string; type: 'success' | 'error'; onClose: () => void }) {
  ...
  return (
    <div className="toast" style={{ background: type === 'success' ? '#0d3b2e' : '#dc2626' }}>
...
export default function VendorAvailabilityPage() {
  ...
  function handleMouseDown(dateStr: string) {
    ...
    setDragValue(willSelect); setIsDragging(true);
    setSelectedDates(prev => { const n = new Set(prev); willSelect ? n.add(dateStr) : n.delete(dateStr); return n; });
  }
  function handleMouseEnter(dateStr: string) {
    if (!isDragging || !canSelect(dateStr)) return;
    setSelectedDates(prev => { const n = new Set(prev); dragValue ? n.add(dateStr) : n.delete(dateStr); return n; });
  }
  ...
  function resetSelected() {
    ...
    setPendingChanges(prev => { const n = { ...prev }; Object.keys(resets).forEach(k => delete n[k]); return n; });
    setSelectedDates(new Set());
    setToast({ msg: `${selectedDates.size} tanggal dikembalikan ke status awal.`, type: 'success' });
  }
  function discardAll() { setPendingChanges({}); setSelectedDates(new Set()); }
    ...
    for (const [status, dates] of Object.entries(byStatus) as [SetStatus, string[]][]) {
      ...
      try {
        ...
        if (data.success) {
          ...
          setServerData(prev => { const n = { ...prev }; dates.forEach(d => { n[d] = status; }); return n; });
        } else errorMsg = data.error ?? 'Terjadi kesalahan.';
      } catch { errorMsg = 'Tidak dapat terhubung ke server.'; }
    }
    setSaveLoading(false);
    setPendingChanges({}); setSelectedDates(new Set());
    setToast(errorMsg ? { msg: errorMsg, type: 'error' } : { msg: `${savedCount} tanggal berhasil disimpan!`, type: 'success' });
  }
  ...
  function getDayStyle(status: DayStatus, isSelected: boolean, todayFlag: boolean): React.CSSProperties {
    const base: React.CSSProperties = {
      ...
      justifyContent: 'center', aspectRatio: '1', borderRadius: 10, fontSize: 13, fontWeight: 500,
      cursor: 'default', userSelect: 'none', transition: 'all 0.12s', border: '2px solid transparent',
    };
    if (status === 'empty') return { ...base };
  ...
  const SidebarContent = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div className="panel-box">
        <p className="panel-label">Ringkasan Bulan</p>
        {[
        ...
        {pendingCount > 0 && (
          <div style={{ marginTop: 6, padding: '8px 10px', background: '#fffbeb', borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div className="panel-box">
        <p className="panel-label">Mode Pengaturan</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {([['blocked', 'Tandai Libur', '#ea580c', '#fff7ed'], ['available', 'Tandai Tersedia', '#16a34a', '#f0fdf4']] as const).map(([mode, label, color, bg]) => (
            <button key={mode} onClick={() => setSelectMode(mode)}
              ...
              <div>
                ...
                <p style={{ fontSize: 11.5, color: '#94a3b8', marginTop: 1 }}>{mode === 'blocked' ? 'Blokir tanggal (istirahat, dll.)' : 'Buka tanggal yang sebelumnya off'}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="panel-box" style={{ borderColor: selectedDates.size > 0 ? '#0d3b2e' : '#f1f5f9' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <p className="panel-label" style={{ margin: 0 }}>Tanggal Dipilih</p>
        ...
        {selectedDates.size === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <button onClick={applyToSelected}
              style={{ width: '100%', padding: '12px 0', borderRadius: 10, background: selectMode === 'blocked' ? '#ea580c' : '#0d3b2e', color: 'white', fontSize: 13.5, fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              <CheckCircle size={15} /> Tandai {selectMode === 'blocked' ? 'Libur' : 'Tersedia'} ({selectedDates.size})
            </button>
  ...
  return (
    <VendorShell>
      <div className="avail-wrap" onMouseUp={() => setIsDragging(false)}>
        ...
        <div className="avail-header">
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              <h1 className="avail-title">Kalender Ketersediaan</h1>
            </div>
            <p style={{ fontSize: 14, color: '#64748b' }}>{vendor.store_name} · Klik atau drag tanggal untuk memilih</p>
          </div>
          <div className="avail-actions">
            {pendingCount > 0 && (
              <button onClick={discardAll} className="btn-discard">
                <RotateCcw size={14} /> <span className="btn-label">Batalkan</span>
              </button>
            )}
            <button onClick={handleSave} disabled={saveLoading || pendingCount === 0} className="btn-save"
              style={{ background: pendingCount === 0 ? '#e5e7eb' : '#0d3b2e', color: pendingCount === 0 ? '#9ca3af' : 'white', cursor: pendingCount === 0 ? 'not-allowed' : 'pointer', boxShadow: pendingCount > 0 ? '0 4px 14px rgba(13,59,46,0.25)' : 'none' }}>
        ...
        {pendingCount > 0 && (
          <div style={{ display: 'flex', gap: 10, padding: '12px 16px', background: '#fffbeb', borderRadius: 12, border: '1px solid #fde68a', marginBottom: 20, alignItems: 'center' }}>
            ...
            <p style={{ fontSize: 13.5, color: '#92400e', flex: 1 }}>
              <strong>{pendingCount} perubahan belum disimpan.</strong> Klik <strong>Simpan</strong> untuk menerapkan.
            </p>
          </div>
        )}
        ...
        {selectedDates.size > 0 && (
          <div className="mobile-selected-bar">
            <span style={{ fontSize: 13, fontWeight: 700, color: '#0f172a' }}>{selectedDates.size} tanggal dipilih</span>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setShowPanel(true)} className="mobile-bar-btn-primary"
                style={{ background: selectMode === 'blocked' ? '#ea580c' : '#0d3b2e' }}>
                Tandai {selectMode === 'blocked' ? 'Libur' : 'Tersedia'}
              </button>
              <button onClick={() => setSelectedDates(new Set())} className="mobile-bar-btn-ghost">
                Batal
              </button>
            </div>
          </div>
        ...
        <div className="avail-layout">
          <div style={{ background: 'white', borderRadius: 20, border: '1px solid #f1f5f9', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid #f1f5f9' }}>
              <button onClick={prevMonth} className="month-nav-btn"><ChevronLeft size={18} color="#374151" /></button>
              <div style={{ textAlign: 'center' }}>
                ...
                {viewDate.getMonth() !== today.getMonth() || viewDate.getFullYear() !== today.getFullYear() ? (
                  <button onClick={goToday} style={{ fontSize: 11.5, color: '#0d3b2e', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginTop: 2 }}>← Ke bulan ini</button>
                ) : (
                  <p style={{ fontSize: 11.5, color: '#94a3b8', marginTop: 2 }}>Bulan ini</p>
                )}
              </div>
              <button onClick={nextMonth} className="month-nav-btn"><ChevronRight size={18} color="#374151" /></button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4, padding: '4px 16px 18px', opacity: fetchLoading ? 0.5 : 1, transition: 'opacity 0.2s' }}>
              {cells.map((day, i) => {
                ...
                return (
                  <div key={i} style={getDayStyle(status, isSelected, todayFlag)}
                    ...
                    {status === 'booked' && <Lock size={9} color="#dc2626" style={{ position: 'absolute', top: 3, right: 3 }} />}
            ...
            <div style={{ display: 'flex', gap: 14, padding: '12px 20px 16px', borderTop: '1px solid #f1f5f9', flexWrap: 'wrap' }}>
              {[
                { color: '#dcfce7', dot: '#16a34a', label: 'Tersedia' },
                { color: '#f3f4f6', dot: '#6b7280', label: 'Libur' },
                { color: '#fee2e2', dot: '#dc2626', label: 'Penuh' },
                { color: '#0d3b2e', dot: 'white',   label: 'Hari ini' },
              ].map(({ color, dot, label }) => (
            ...
            <div className="mobile-mode-bar">
              {([['blocked', 'Tandai Libur', '#ea580c'], ['available', 'Tandai Tersedia', '#16a34a']] as const).map(([mode, label, color]) => (
                <button key={mode} onClick={() => setSelectMode(mode)}
                  style={{ flex: 1, padding: '10px 0', borderRadius: 10, border: `2px solid ${selectMode === mode ? color : '#e5e7eb'}`, background: selectMode === mode ? (mode === 'blocked' ? '#fff7ed' : '#f0fdf4') : 'white', color: selectMode === mode ? color : '#64748b', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.18s' }}>
                  {label}
                </button>
              ))}
            </div>
        ...
        {showPanel && (
          <div className="mobile-sheet-backdrop" onClick={() => setShowPanel(false)}>
      ...
      <style>{`
        ...
        .avail-title { font-size: 24px; font-weight: 800; color: #0f172a; letter-spacing: -0.4px; }
        .avail-actions { display: flex; gap: 10px; align-items: center; flex-shrink: 0; }
        .btn-discard {
          display: flex; align-items: center; gap: 6px;
          padding: 10px 16px; border-radius: 10px;
          background: white; color: #64748b; font-size: 13.5px;
        ...
        .avail-sidebar { display: flex; flex-direction: column; gap: 16px; }
        ...
        .month-nav-btn {
          ...
          display: grid; place-items: center; cursor: pointer; transition: all 0.18s;
        }

        .mobile-mode-bar      { display: none; }
        .mobile-selected-bar  { display: none; }
        ...
        @media (max-width: 860px) {
          ...
          .avail-sidebar         { display: none !important; }
          .mobile-mode-bar       {
          ...
          .mobile-selected-bar   {
            ...
            border-radius: 14px; border: 1.5px solid #0d3b2e;
          ...
          .mobile-sheet {
            ...
            padding: 0 20px 40px; animation: sheetUp 0.3s cubic-bezier(0.22,1,0.36,1);
          }
          .avail-wrap { padding: 80px 16px 80px; }
          .toast { right: 16px; bottom: 16px; left: 16px; max-width: unset; }
        }

        @media (max-width: 640px) {
          .avail-title    { font-size: 20px; }
          .avail-header   { gap: 12px; }
          .btn-label      { display: none; }
          .btn-discard    { padding: 10px 12px; }
          .btn-save       { padding: 11px 14px; font-size: 13px; }
          .avail-wrap     { padding: 72px 12px 80px; }
